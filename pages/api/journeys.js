...
async function getBigQueryJourneys(from, to, url, page) {
  ...
  const urlFilter = url
    ...
    : ''
  const fullFilter = dateFilter + urlFilter
  const params = url ? { from, to, url } : { from, to }

  const [[countRows], [sessionRows], [pageListRows]] = await Promise.all([
    bq.query({
      query: `SELECT COUNT(DISTINCT session_id) AS total FROM \`${ds}.${tbl}\` WHERE ${fullFilter}`,
      params,
    }),
    bq.query({
      query: `
...
function getMemoryJourneys(from, to, url, page) {
...
export default async function handler(req, res) {
  ...
  try {
    const data = bigQueryConfigured()
      ? await getBigQueryJourneys(from, to, url, page)
      : getMemoryJourneys(from, to, url, page)
    return res.status(200).json(data)
  } catch {
    return res.status(200).json(getMemoryJourneys(from, to, url, page))
  }
}
